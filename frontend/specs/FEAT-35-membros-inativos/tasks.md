...
- [ ] 1. `errors/memberErrors.ts`: adicionar `CannotModifyInactiveMemberError`
...
- [ ] 2. `api/membersApi.ts`: `MemberStatus` ganha `'Inativo'`;
...
- [ ] 3. `hooks/useUpdateMemberRole.test.ts`: novo caso "422
...
- [ ] 8. `hooks/useMembers.test.ts`: novos casos — `refetch()` atualiza
...
- [ ] 9. `components/MemberRow.tsx`: `isEffectivelyReadOnly = readOnly
...
- [ ] 10. `components/MemberRow.test.tsx`: novos casos — membro
...
- [ ] 11. `components/MemberRemoveDialog.tsx`: atualizar o texto de
...
- [ ] 12. `components/MemberRemoveDialog.test.tsx`: atualizar a
...
- [ ] 13. `routes/MembersPage.tsx`: `handleRemoved` chama `refetch()`
...
- [ ] 14. `routes/MembersPage.test.tsx`: atualizar "Titular remove um
...
- [ ] 15. `routes/MembersPage.test.tsx`: novo caso — durante o
...
- [ ] 16. `hooks/useInviteMember.test.ts` (ou
...
- [ ] 17. `frontend/specs/FEAT-35-membros-inativos/spec.md`: marcar os
      critérios de aceite concluídos (checklist `- [x]`).

- [ ] 18. Rodar suíte completa (`npm run lint`, `npm test`,

...
- [x] 1. Atualizar `styles/modernist/modernist.css`: novas classes
      ...
      do bundle base)
- [x] 2. Criar `lib/auth/currentUserApi.ts` (tipo `CurrentUser`,
...
- [x] 3. Criar `lib/auth/useCurrentUser.ts` (mesmo esqueleto de
...
- [x] 4. Criar `components/Toast.tsx` e `Toast.test.tsx`: renderiza a
...
- [x] 5. Criar `components/ProcessingOverlay.tsx` e
      `ProcessingOverlay.test.tsx`: renderiza o `label` recebido e o
      spinner/barra indeterminada
- [x] 6. Criar `features/members/api/membersApi.ts`: tipos
...
- [x] 7. Criar `features/members/errors/memberErrors.ts`:
      ...
      `UnknownMemberError`
- [x] 8. Criar `features/members/utils/roleLabels.ts`
      (`ROLE_LABEL`/`ROLE_DESCRIPTION`) e `roleLabels.test.ts` cobrindo
      os 4/3 mapeamentos
- [x] 9. Criar `features/members/hooks/useMembers.ts` (`GET /members`,
...
- [x] 10. Criar `features/members/hooks/useInviteMember.ts` (`POST
      ...
      limpa a authStore
- [x] 11. Criar `features/members/hooks/useUpdateMemberRole.ts` (`PUT
...
- [x] 12. Criar `features/members/hooks/useRemoveMember.ts` (`DELETE
...
- [x] 13. Criar `features/members/components/MemberRow.tsx` e
...
- [x] 14. Criar `features/members/components/MemberList.tsx` e
      ...
      por linha
- [x] 15. Criar `features/members/components/MemberRemoveDialog.tsx` e
...
- [x] 16. Criar `features/members/components/InviteMemberDialog.tsx` e
      ...
      fechar o popup
- [x] 17. Criar `routes/MembersPage.tsx`: busca `useMembers` +
...
- [x] 18. Criar `routes/MembersPage.test.tsx` cobrindo: Titular vê
      ...
      sessão expirada
- [x] 19. Atualizar `app/router.tsx`: nova rota `{ path: 'members',
      element: <MembersPage /> }`
- [x] 20. Atualizar `components/nav/navConfig.ts`: novo item `members`
      ("Membros"), entre `categories` e `settings`
- [x] 21. Rodar a suíte completa (`npm test`), `tsc -b`, `oxlint` e
      `npm run build`; confirmar 100% dos testes passando, sem erro de
      tipo e sem warning novo de lint
- [x] 22. Revisão manual/visual: no app real (backend local +
      ...
      `showInviteDialog`)
- [x] 23. Atualizar `spec.md` marcando todos os critérios de aceite
      concluídos (`- [x]`)

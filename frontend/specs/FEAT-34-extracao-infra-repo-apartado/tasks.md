...
- [x] 1. [infra] Acrescentar `.terraform.lock.hcl` ao `.gitignore`
      (template do GitHub não cobre; prática do monorepo mantida)
- [x] 2. [infra] Criar `CLAUDE.md`: governança (apply manual/local,
...
- [x] 3. [infra] Criar `README.md`: propósito do repo, layout
      ...
      seção `cicd/`
- [x] 4. [infra] Criar `scripts/migration/.gitkeep` e
...
- [x] 8. [mono] Baseline: `terraform init` + `terraform plan` em
...
- [x] 9. [infra] Criar `terraform/dns/`: `route53.tf` e
...
- [x] 10. [infra] `terraform init` em `terraform/dns/` (state remoto
      vazio) e `terraform validate`
- [x] 11. [infra] Criar `scripts/migration/wave-2-dns.sh` com o loop de
...
- [x] 19. [mono] Baseline: `terraform plan` em
...
- [x] 20. [infra] Criar `terraform/environments/hom/`: `versions.tf`
...
- [x] 21. [infra] `terraform init` + `terraform validate` em
      `terraform/environments/hom/`
- [x] 22. [infra] Criar `scripts/migration/wave-3-frontend-hom.sh`
...
- [x] 23. Scratch: `state pull` do monorepo hom → `mono.tfstate` +
      backup; `state pull` da infra hom → `infra.tfstate`; rodar o
      script; conferir `state list` dos dois arquivos
- [x] 24. [infra] **[APROVAÇÃO]** `state push infra.tfstate` em
      ...
      `origin.domain_name`
- [x] 25. [infra] **[APROVAÇÃO]** `terraform apply` só de outputs;
      `terraform plan` = "No changes"; `terraform output
      cloudfront_distribution_arn` bate com o ARN de `ELE195A1APCLB`
- [x] 26. [mono] Editar `frontend/infra/terraform/environments/hom/`:
      ...
      `terraform validate`
- [x] 27. [mono] **[APROVAÇÃO]** `state push mono.tfstate` em
      `environments/hom/`; `terraform plan` → esperado nenhum recurso
      (bucket policy sem diff) e só outputs removidos/adicionados
- [x] 28. [mono] **[APROVAÇÃO]** `terraform apply` só de outputs;
      `terraform plan` = "No changes"
- [x] 29. [infra] `terraform/dns/variables.tf`: `hom_state_key` →
      `infra-jrnexpenses/hom/terraform.tfstate`; `terraform plan` em
      `dns/` = "No changes"
- [x] 30. Smoke manual: `https://hom.jrnexpenses.com` carrega, F5 em
...
- [x] 34. [mono] Baseline: `terraform plan` em `environments/prod/` =
      "No changes"; `state list` e `state show
      aws_cloudfront_distribution.main` salvos
- [x] 35. [infra] Criar `terraform/environments/prod/`: `versions.tf`
...
- [x] 36. [infra] `terraform init` + `terraform validate` em
      `environments/prod/`
- [x] 37. [infra] Criar `scripts/migration/wave-4-frontend-prod.sh`
...
- [x] 38. Scratch: pulls + backup, rodar o script, conferir `state list`
- [x] 39. [infra] **[APROVAÇÃO]** `state push` em `environments/prod/`;
      `terraform plan` revisado **linha a linha** — nenhum recurso,
      só outputs; distribuição `E2YCZNS0F94SCU` sem diff
- [x] 40. [infra] **[APROVAÇÃO]** `apply` só de outputs; `plan` = "No
      changes"
- [x] 41. [mono] Editar `environments/prod/` (mesmas mudanças da task
      26, `variables.tf` remove `domain_name`, `infra_state_key` default
      `infra-jrnexpenses/prod/terraform.tfstate`); `terraform validate`
- [x] 42. [mono] **[APROVAÇÃO]** `state push` em `environments/prod/`;
      `plan` → nenhum recurso, só outputs
- [x] 43. [mono] **[APROVAÇÃO]** `apply` só de outputs; `plan` = "No
      changes"
- [x] 44. [infra] `dns/variables.tf`: `prod_state_key` →
      `infra-jrnexpenses/prod/terraform.tfstate`; `plan` em `dns/` =
      "No changes"
- [x] 45. Smoke manual em `https://jrnexpenses.com` e
      `https://www.jrnexpenses.com` (carrega, F5 em rota interna,
      certificado válido)
- [x] 46. [mono] `frontend/infra/terraform/README.md` e
      `frontend/infra/CLAUDE.md` atualizados para prod; commit
- [x] 47. [infra] Commit "feat(prod): migra CloudFront/OAC/ACM/WAF do
      frontend (FEAT-34 etapa 4)"; push

## Etapa 7 (parte do frontend) — fechamento

- [x] 48. [mono] Reescrita final de `frontend/infra/CLAUDE.md` e
...
- [x] 49. [mono] Conferir que nada em `.github/workflows/*` nem nos
      GitHub Environments `hom`/`prod` foi alterado (`git diff develop
      -- .github/` vazio)
- [x] 50. [mono] Atualizar `spec.md` marcando os critérios de aceite
      concluídos (`- [x]`) e adicionar seção "Status" com data, o que
      foi movido, states órfãos pendentes de limpeza e achados
- [x] 51. [mono] `frontend/docs/backlog.md`: marcar FEAT-34 como
      concluída; commit
- [x] 52. [infra] Abrir PR `develop → main` no `infra-jrnexpenses`
      (manual) com resumo das 3 migrações de state; merge manual pelo
      usuário
- [x] 53. [mono] Abrir PR `FEAT-34-extracao-infra-repo-apartado → develop`

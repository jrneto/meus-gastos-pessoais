...
- [ ] 1. Mover/renomear toda a pasta `features/expenses/` para
...
- [ ] 2. Atualizar `features/transactions/api/transactionsApi.ts`:
...
- [ ] 3. Atualizar `features/transactions/errors/transactionErrors.ts`:
...
- [ ] 4. Atualizar `features/transactions/schemas/transactionSchema.ts`
...
- [ ] 6. Atualizar `features/transactions/hooks/useRegisterTransaction.ts`
      e seu teste: importar de `transactionsApi`/`transactionErrors`;
      montar o payload com `tipo: 'despesa'` fixo e `date` no lugar de
      `expenseDate`
- [ ] 7. Atualizar `features/transactions/hooks/useUpdateTransaction.ts`
      e seu teste: mesma mudança de payload (`tipo: 'despesa'` fixo,
      `date`) e imports
- [ ] 8. Atualizar `features/transactions/hooks/useTransactionsQuery.ts`,
...
- [ ] 9. Atualizar `features/transactions/components/TransactionForm.tsx`
...
- [ ] 11. Atualizar `features/transactions/components/TransactionList.tsx`
...
- [ ] 15. Renomear `routes/ExpensesListPage.tsx` para
...
- [ ] 16. Atualizar `app/router.tsx`: path `expenses`→`transactions`,
      import de `TransactionsListPage` no lugar de `ExpensesListPage`
- [ ] 17. Atualizar `components/nav/navConfig.ts` e
      `navConfig.test.ts`: `id: 'expenses'`→`'transactions'`,
      `to: '/expenses'`→`'/transactions'` (label "Transações" já
      correto, sem mudança)
- [ ] 18. Atualizar `components/nav/DesktopSidebar.test.tsx`,
...
- [ ] 19. Atualizar os comentários em
...
- [ ] 20. Rodar a suíte completa (`npm test`), `tsc -b`, `oxlint` e

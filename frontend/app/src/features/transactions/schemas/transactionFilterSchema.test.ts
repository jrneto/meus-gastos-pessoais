...
describe('transactionFilterSchema', () => {
  ...
  it('aceita apenas yearMonth informado', () => {
    const result = transactionFilterSchema.safeParse({ yearMonth: '2025-06' })
    expect(result.success).toBe(true)
    if (result.success) {
  ...
  it('aceita apenas categoryId informado', () => {
    const result = transactionFilterSchema.safeParse({ categoryId: 'cat-1' })
    expect(result.success).toBe(true)
    if (result.success) {
  ...
  it('converte minAmount/maxAmount para centavos', () => {
    const result = transactionFilterSchema.safeParse({ minAmount: '10,00', maxAmount: '100,00' })
    expect(result.success).toBe(true)
    if (result.success) {
  ...
  it('rejeita minAmount em formato inválido', () => {
    const result = transactionFilterSchema.safeParse({ minAmount: 'dez reais' })
    expect(result.success).toBe(false)
  })

  it('aceita dateFrom e dateTo válidos (intervalo consistente)', () => {
    const result = transactionFilterSchema.safeParse({ dateFrom: '2025-06-01', dateTo: '2025-06-30' })
    expect(result.success).toBe(true)
  })

  it('rejeita dateFrom posterior a dateTo', () => {
    const result = transactionFilterSchema.safeParse({ dateFrom: '2025-06-30', dateTo: '2025-06-01' })
    expect(result.success).toBe(false)
    if (!result.success) {
  ...
  it('rejeita minAmount maior que maxAmount', () => {
    const result = transactionFilterSchema.safeParse({ minAmount: '100,00', maxAmount: '10,00' })
    expect(result.success).toBe(false)
    if (!result.success) {

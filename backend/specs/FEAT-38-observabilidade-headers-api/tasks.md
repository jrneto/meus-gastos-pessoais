...
- [ ] 1. Criar `ObservabilityHeaderNames` (`backend/src/GastosApp.Api/Common/ObservabilityHeaderNames.cs`) — constantes `TraceId = "trace-id"`, `SessionId = "session-id"`, `ClientPlatform = "client-platform"`, `ClientVersion = "client-version"`

- [ ] 2. Criar `LoggingOptions` (`backend/src/GastosApp.Infrastructure/Configuration/LoggingOptions.cs`) — `SectionName = "Logging"`, propriedade `FullPayloadLoggingEnabled` (bool)

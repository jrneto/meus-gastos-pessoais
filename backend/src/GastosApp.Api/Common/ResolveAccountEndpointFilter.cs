...
// Aplicado nos grupos /categories e /expenses (depois de
// .RequireAuthorization()). Resolve o accountId do userId autenticado
// uma única vez por request, evitando duplicar essa chamada em cada
// endpoint — e garante que ausência de conta vira sempre 401
// (account-not-found), nunca um erro esquecido em algum handler.
public sealed class ResolveAccountEndpointFilter : IEndpointFilter
{
    ...
    public async ValueTask<object?> InvokeAsync(EndpointFilterInvocationContext context, EndpointFilterDelegate next)
    {
        ...
        var result = await _sender.Send(new ResolveAccountIdQuery(userId), context.HttpContext.RequestAborted);
        if (result.IsFailure)
            return result.ToHttpResult(_ => Results.Ok());

        _currentAccount.AccountId = result.Value;

...
public static class TransactionCursorCodec
{
    private static readonly string[] ValidIndexes = ["Base", "GSI1"];

    public static string Encode(TransactionCursorPayload payload)
    {
        var json = JsonSerializer.Serialize(payload, TransactionCursorJsonContext.Default.TransactionCursorPayload);
        var bytes = Encoding.UTF8.GetBytes(json);
        return Base64UrlEncode(bytes);
    }

    public static bool TryDecode(string cursor, out TransactionCursorPayload? payload)
    {
        payload = null;
        ...
        try
        {
            ...
            var decoded = JsonSerializer.Deserialize(json, TransactionCursorJsonContext.Default.TransactionCursorPayload);

#!/usr/bin/env bash
# Sobe o ambiente local completo pros testes integrados (FEAT-29) e
# DEIXA RODANDO — builda a imagem Native AOT, garante LocalStack +
# cognito-local no ar, e sobe o container da Api via Lambda Runtime
# Interface Emulator em http://localhost:9000. Não roda nenhum teste,
# não desliga nada ao terminar — feito pra ficar no ar enquanto você
# debuga a suíte no VS Code (ver
# backend/tests/GastosApp.IntegrationTests/README.md, "Debugar com
# breakpoint no VS Code").
#
# `run-local.sh` reaproveita este script (up + testes + down); use este
# aqui direto quando quiser controlar o ciclo de vida do container você
# mesmo. Para desligar depois: ./infra/lambda/local-env-down.sh
#
...
#
# Sem "--rm" de propósito (diferente de run-local.sh) — este script não
# desliga o container ao sair, ele deve continuar no ar pra você debugar
# contra ele. Use local-env-down.sh quando terminar.
docker run -d \
  --name "$CONTAINER_NAME" \
...
Ambiente local no ar:
  API (via RIE):   http://localhost:9000
  LocalStack:      http://localhost:4566
  cognito-local:   http://localhost:9229

Agora é só rodar/debugar GastosApp.IntegrationTests com
INTEGRATION_TESTS_MODE=local (ver README.md, "Debugar com breakpoint
no VS Code"). Quando terminar:
